...
class Coin(pygame.sprite.Sprite):
    def __init__(self):
        ...
        coinW = random.randint(30, 60) # рандомизация ширины монеты
        self.image = pygame.Surface((coinW, 30))  # сама монета
...
while True:
    for event in pygame.event.get():
        if event.type == INC_SPEED and COIN_COUNT % N == 0 and COIN_COUNT != 0:
            SPEED += 0.5
        if event.type == QUIT:
            pygame.quit()
            sys.exit()
    ...
    # движения (если кто то передвинулся обновляет экран типа)
